...
static int w600_protect_check(struct flash_bank *bank)
{
	return ERROR_OK;
}

static int w600_protect(struct flash_bank *bank, int set, int first, int last)
{
	return ERROR_FAIL;
}
		...
		uint32_t offset, uint32_t count)
{
	...
	int cur_size;
	uint8_t localbuffer[W600_FLASH_PAGESIZE];
	while (count > 0)
	{
		cur_size = count > W600_FLASH_PAGESIZE ? W600_FLASH_PAGESIZE : count;

		memcpy(localbuffer, buffer, cur_size);
		if (cur_size < W600_FLASH_PAGESIZE)
			memset(&localbuffer[cur_size], 0xFF, W600_FLASH_PAGESIZE - cur_size);

		retval = target_write_buffer(target, QFLASH_BUFFER, W600_FLASH_PAGESIZE, localbuffer);
		if (retval != ERROR_OK)
			break;
...
COMMAND_HANDLER(w600_handle_mass_erase_command)
{
	if (CMD_ARGC < 1)
		return ERROR_COMMAND_SYNTAX_ERROR;
	...
	retval = w600_erase(bank, W600_FLASH_PROTECT_SIZE / W600_FLASH_SECSIZE, bank->num_sectors  - 1);
	if (retval == ERROR_OK) {
		/* set all sectors as erased */
		for (int i = 0; i < bank->num_sectors; i++)
			bank->sectors[i].is_erased = 1;

		command_print(CMD_CTX, "w600 mass erase complete");
	} else
		command_print(CMD_CTX, "w600 mass erase failed");

	return retval;
}

static const struct command_registration w600_exec_command_handlers[] = {
	{
		.name = "mass_erase",
		.handler = w600_handle_mass_erase_command,
		.mode = COMMAND_EXEC,
		.usage = "bank_id",
...
static const struct command_registration w600_command_handlers[] = {
	{
		.name = "w600",
		.mode = COMMAND_ANY,
		.help = "w600 flash command group",
		.usage = "",
		.chain = w600_exec_command_handlers,
	},
	COMMAND_REGISTRATION_DONE
};
...
struct flash_driver w600_flash = {
	.name = "w600",
	.commands = w600_command_handlers,
	.flash_bank_command = w600_flash_bank_command,
	.erase = w600_erase,
	.protect = w600_protect,
	.write = w600_write,
	.read = default_flash_read,
	.probe = w600_probe,
	.auto_probe = w600_auto_probe,
	.erase_check = default_flash_blank_check,
	.protect_check = w600_protect_check,
	.info = get_w600_info,
